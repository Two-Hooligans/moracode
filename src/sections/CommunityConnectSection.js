...
export default function CommunityConnectSection() {
  ...
  itemRefs.current = communityConnectItems.map(
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      scrollRef.current = Math.max(
        0,
        Math.min(rawScroll, totalAnimationHeight)
      );
      ...
      itemRefs.current.forEach((ref, i) => {
        ...
        const progress = Math.max(
          0,
          Math.min(1, (scrollRef.current - start) / (end - start))
        );

        const newHeight =
          maxItemHeight - (maxItemHeight - minItemHeight) * progress;
            ...
            >
              <div className="flex flex-col w-full md:w-2/5 py-2 justify-center">
                <div className="text-to-fade flex-grow">
                  <h3 className="mb:text-[32px] text-[26px] mb-1 text-[#252525]">
                    {item.title}
                  </h3>
                  <p className="text-md text-[#252525] mb-4">{item.desc}</p>
                </div>
                <div>
                  <button className="rounded-md px-[26px] py-[13px] text-[#252525] bg-[#D2F944] border border-[#7e7e7e] button-fill-effect group">
